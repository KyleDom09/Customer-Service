    ...
    <header class="h-20 bg-white dark:bg-[#1e1e1e] flex items-center justify-between px-4 sm:px-8 sticky top-0 z-20 border-b border-slate-100 dark:border-slate-700 transition-colors">
      ...
      <div class="flex items-center gap-4 sm:gap-5">
        ...
        <div class="flex items-center gap-2 cursor-pointer">
          ...
          <span class="hidden sm:block text-sm font-semibold text-slate-700 dark:text-slate-100">Benju Guzman</span>
...
<div id="viewModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
  ...
  <div class="relative bg-white dark:bg-[#1e1e1e] w-full max-w-2xl max-h-[85vh] rounded-3xl shadow-xl flex flex-col overflow-hidden">
...
<div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
  ...
  <div class="relative bg-white dark:bg-[#1e1e1e] w-full max-w-lg rounded-3xl shadow-xl flex flex-col overflow-hidden">
    ...
    <div class="flex items-center justify-between px-6 sm:px-8 py-5 border-b border-slate-100 dark:border-slate-700">
      <div>
        <p class="font-semibold text-slate-800 dark:text-slate-100">Edit Communication</p>
    ...
    <form id="editForm" class="px-6 sm:px-8 py-6 space-y-4">
      ...
      <div>
        ...
        <input id="editResp" type="text" readonly placeholder="Calculated automatically" class="w-full border border-[#E5E7EB] rounded-xl px-3 py-2 text-sm bg-slate-50 text-slate-500 focus:outline-none">
        <p class="text-[11px] text-slate-400 mt-1">Auto-calculated when status is set to Completed or Resolved.</p>
      </div>

      <div class="flex items-center justify-end gap-3 pt-2">
        <button type="button" id="editCancelBtn" class="px-4 py-2.5 rounded-xl border border-[#CBD5E1] text-slate-600 text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">Cancel</button>
  ...
  function renderPage() {
    ...
    tbody.innerHTML = pageRows.map(r => `
      <tr class="table-row-fx hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all duration-200">
        ...
        <td class="px-4 py-4 text-slate-500 dark:text-slate-400 whitespace-nowrap">${r.resp}</td>
        <td class="px-8 py-4">
          <div class="flex items-center gap-2">
            ...
            <button class="edit-btn px-3 py-1.5 rounded-lg border border-[#CBD5E1] text-slate-600 text-xs font-semibold hover:bg-slate-50 transition-colors whitespace-nowrap" data-id="${r.id}">Edit</button>
          </div>
        </td>
      </tr>
    `).join('');
